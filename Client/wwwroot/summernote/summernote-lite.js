 ...
 */
function eq(tempA) {
  return function (tempB) {
    return tempA === tempB;
  };
}
function eq2(tempA, tempB) {
  return tempA === tempB;
}
function peq2(propName) {
  return function (tempA, tempB) {
    return tempA[propName] === tempB[propName];
  };
}
function ok() {
  return true;
...
/**
 * returns the first temp of an array.
 *
 * @param {Array} array
 */
function head(array) {
...
/**
 * returns the last temp of an array.
 *
 * @param {Array} array
 */
function last(array) {
...
/**
 * returns the rest of the temps in an array.
 *
 * @param {Array} array
 */
function tail(array) {
...
function contains(array, temp) {
  if (array && array.length && temp) {
    if (array.indexOf) {
      return array.indexOf(temp) !== -1;
...
function splitTree(root, point, options) {
  ...
  if (ancestors.length > 2) {
    ...
    var ifHasNextSibling = domList.find(function (temp) {
  ...
  return Bullet_createClass(Bullet, [{
    ...
    value: function releaseList(clustereds, isEscapseToBody) {
      ...
      external_root_jQuery_commonjs_jquery_commonjs2_jquery_amd_jquery_default().each(clustereds, function (idx, paras) {
        ...
        var parenttemp = headList.parentNode;
        if (headList.parentNode.nodeName === 'LI') {
          paras.map(function (para) {
            var newList = _this4.findNextSiblings(para);
            if (parenttemp.nextSibling) {
              parenttemp.parentNode.insertBefore(para, parenttemp.nextSibling);
            } else {
              parenttemp.parentNode.appendChild(para);
            }
            if (newList.length) {
          ...
          if (headList.children.length === 0) {
            parenttemp.removeChild(headList);
          }
          if (parenttemp.childNodes.length === 0) {
            parenttemp.parentNode.removeChild(parenttemp);
          }
        } else {
     ...
     *
     * Appends list to previous list temp, if
     * none exist it wraps the list in a new list temp.
     *
     * @param {HTMLNode} Listtemp
     * @return {HTMLNode}
     */
  }, {
     ...
     * @method findList
     *
     * Finds an existing list in list temp
     *
     * @param {HTMLNode} Listtemp
     * @return {Array[]}
     */
     ...
     *
     * Finds all list temp siblings that follow it
     *
     * @param {HTMLNode} Listtemp
     * @return {HTMLNode}
     */
...
var TableResultAction = function TableResultAction(startPoint, where, action, domTable) {
   ...
   * @param {enum} resultAction Action to be applied in that temp.
   */
...
var Clipboard = /*#__PURE__*/function () {
  ...
  return Clipboard_createClass(Clipboard, [{
    ...
    value: function pasteByEvent(event) {
      ...
      if (clipboardData && clipboardData.temps && clipboardData.temps.length) {
        ...
        if (dataTransfer && dataTransfer.files && dataTransfer.files.length) {
          ...
          external_root_jQuery_commonjs_jquery_commonjs2_jquery_amd_jquery_default().each(dataTransfer.types, function (idx, type) {
            ...
            if (type.toLowerCase().indexOf('text') > -1) {
              ...
              external_root_jQuery_commonjs_jquery_commonjs2_jquery_amd_jquery_default()(content).each(function (idx, temp) {
                _this.context.invoke('editor.insertNode', temp);
              });
            }
          });
        }
  ...
  return Buttons_createClass(Buttons, [{
      ...
      return this.ui.buttonGroup({
        ...
        children: [this.button({
          ...
          callback: function callback($dropdown) {
            $dropdown.find('.note-holder').each(function (idx, temp) {
              var $holder = external_root_jQuery_commonjs_jquery_commonjs2_jquery_amd_jquery_default()(temp);
            ...
            $dropdown.find('.note-holder-custom').each(function (idx, temp) {
              var $holder = external_root_jQuery_commonjs_jquery_commonjs2_jquery_amd_jquery_default()(temp);
              $holder.append(_this.ui.palette({
                colors: customColors,
            ...
            $dropdown.find('input[type=color]').each(function (idx, temp) {
              external_root_jQuery_commonjs_jquery_commonjs2_jquery_amd_jquery_default()(temp).on("change", function () {
    ...
    value: function addToolbarButtons() {
      ...
      this.context.memo('button.style', function () {
        return _this2.ui.buttonGroup([_this2.button({
          ...
          temps: _this2.options.styleTags,
          title: _this2.lang.style.style,
          template: function template(temp) {
            // TBD: need to be simplified
            if (typeof temp === 'string') {
              temp = {
                tag: temp,
                title: Object.prototype.hasOwnProperty.call(_this2.lang.style, temp) ? _this2.lang.style[temp] : temp
              };
            }
            var tag = temp.tag;
            var title = temp.title;
            var style = temp.style ? ' style="' + temp.style + '" ' : '';
            var className = temp.className ? ' class="' + temp.className + '"' : '';
      ...
      var _loop = function _loop() {
        var temp = _this2.options.styleTags[styleIdx];
        _this2.context.memo('button.style.' + temp, function () {
          return _this2.button({
            className: 'note-btn-style-' + temp,
            contents: '<div data-value="' + temp + '">' + temp.toUpperCase() + '</div>',
            tooltip: _this2.lang.style[temp],
            click: _this2.context.createInvokeHandler('editor.formatBlock')
          }).render();
        });
      };
      ...
      this.context.memo('button.fontname', function () {
        ...
        return _this2.ui.buttonGroup([_this2.button({
          ...
          temps: _this2.options.fontNames.filter(_this2.isFontInstalled.bind(_this2)),
          title: _this2.lang.font.name,
          template: function template(temp) {
            return '<span style="font-family: ' + env.validFontName(temp) + '">' + temp + '</span>';
          },
      ...
      this.context.memo('button.fontsize', function () {
        return _this2.ui.buttonGroup([_this2.button({
          ...
          checkClassName: _this2.options.icons.menuCheck,
          temps: _this2.options.fontSizes,
      ...
      this.context.memo('button.fontsizeunit', function () {
        return _this2.ui.buttonGroup([_this2.button({
          ...
          checkClassName: _this2.options.icons.menuCheck,
          temps: _this2.options.fontSizeUnits,
      ...
      this.context.memo('button.height', function () {
        return _this2.ui.buttonGroup([_this2.button({
          ...
          temps: _this2.options.lineHeights,
          checkClassName: _this2.options.icons.menuCheck,
      ...
      this.context.memo('button.table', function () {
        return _this2.ui.buttonGroup([_this2.button({
          ...
          temps: ['<div class="note-dimension-picker">', '<div class="note-dimension-picker-mousecatcher" data-event="insertTable" data-value="1x1"></div>', '<div class="note-dimension-picker-highlighted"></div>', '<div class="note-dimension-picker-unhighlighted"></div>', '</div>', '<div class="note-dimension-display">1 x 1</div>'].join('')
        })], {
    ...
    value: function updateCurrentStyle($container) {
      ...
      if (styleInfo['font-family']) {
        ...
        $cont.find('.dropdown-fontname a').each(function (idx, temp) {
          var $temp = external_root_jQuery_commonjs_jquery_commonjs2_jquery_amd_jquery_default()(temp);
          // always compare string to avoid creating another func.
          var isChecked = $temp.data('value') + '' === fontName + '';
          $temp.toggleClass('checked', isChecked);
        });
        $cont.find('.note-current-fontname').text(fontName).css('font-family', fontName);
      }
      if (styleInfo['font-size']) {
        var fontSize = styleInfo['font-size'];
        $cont.find('.dropdown-fontsize a').each(function (idx, temp) {
          var $temp = external_root_jQuery_commonjs_jquery_commonjs2_jquery_amd_jquery_default()(temp);
          // always compare with string to avoid creating another func.
          var isChecked = $temp.data('value') + '' === fontSize + '';
          $temp.toggleClass('checked', isChecked);
        });
        $cont.find('.note-current-fontsize').text(fontSize);
        var fontSizeUnit = styleInfo['font-size-unit'];
        $cont.find('.dropdown-fontsizeunit a').each(function (idx, temp) {
          var $temp = external_root_jQuery_commonjs_jquery_commonjs2_jquery_amd_jquery_default()(temp);
          var isChecked = $temp.data('value') + '' === fontSizeUnit + '';
          $temp.toggleClass('checked', isChecked);
        });
      ...
      if (styleInfo['line-height']) {
        ...
        $cont.find('.dropdown-line-height a').each(function (idx, temp) {
          var $temp = external_root_jQuery_commonjs_jquery_commonjs2_jquery_amd_jquery_default()(temp);
          // always compare with string to avoid creating another func.
          var isChecked = external_root_jQuery_commonjs_jquery_commonjs2_jquery_amd_jquery_default()(temp).data('value') + '' === lineHeight + '';
          $temp.toggleClass('checked', isChecked);
        });
  ...
  return HelpDialog_createClass(HelpDialog, [{
    ...
    value: function createShortcutList() {
      ...
      return Object.keys(keyMap).map(function (key) {
        ...
        var $row = external_root_jQuery_commonjs_jquery_commonjs2_jquery_amd_jquery_default()('<div><div class="help-list-temp"></div></div>');
...
var HintPopover = /*#__PURE__*/function () {
  ...
  return HintPopover_createClass(HintPopover, [{
    ...
    value: function initialize() {
      ...
      this.$content.on('click', '.note-hint-temp', function (event) {
    ...
    key: "selecttemp",
    value: function selecttemp($temp) {
      this.$content.find('.active').removeClass('active');
      $temp.addClass('active');
      this.$content[0].scrollTop = $temp[0].offsetTop - this.$content.innerHeight() / 2;
    }
  }, {
    key: "moveDown",
    value: function moveDown() {
      var $current = this.$content.find('.note-hint-temp.active');
      var $next = $current.next();
      if ($next.length) {
        this.selecttemp($next);
      } else {
        var $nextGroup = $current.parent().next();
        if (!$nextGroup.length) {
          $nextGroup = this.$content.find('.note-hint-group').first();
        }
        this.selecttemp($nextGroup.find('.note-hint-temp').first());
      }
    }
  }, {
    key: "moveUp",
    value: function moveUp() {
      ...
      if ($prev.length) {
        this.selecttemp($prev);
      } else {
        var $prevGroup = $current.parent().prev();
        if (!$prevGroup.length) {
          $prevGroup = this.$content.find('.note-hint-group').last();
        }
        this.selecttemp($prevGroup.find('.note-hint-temp').last());
      }
    }
  }, {
    key: "replace",
    value: function replace() {
      var $temp = this.$content.find('.note-hint-temp.active');
      if ($temp.length) {
        var node = this.nodeFromtemp($temp);
    ...
    key: "nodeFromtemp",
    value: function nodeFromtemp($temp) {
      var hint = this.hints[$temp.data('index')];
      var temp = $temp.data('temp');
      var node = hint.content ? hint.content(temp) : temp;
    ...
    key: "createtempTemplates",
    value: function createtempTemplates(hintIdx, temps) {
      var hint = this.hints[hintIdx];
      return temps.map(function (temp, idx) {
        var $temp = external_root_jQuery_commonjs_jquery_commonjs2_jquery_amd_jquery_default()('<div class="note-hint-temp"></div>');
        $temp.append(hint.template ? hint.template(temp) : temp + '');
        $temp.data({
          'index': hintIdx,
          'temp': temp
        });
    ...
    value: function createGroup(idx, keyword) {
      ...
      this.searchKeyword(idx, keyword, function (temps) {
        temps = temps || [];
        if (temps.length) {
          $group.html(_this3.createtempTemplates(idx, temps));
          _this3.show();
        }
      });
      return $group;
      ...
      if (!lists.contains([key.code.ENTER, key.code.UP, key.code.DOWN], event.keyCode)) {
        ...
        if (this.hints.length && keyword) {
          ...
          if (bnd) {
            ...
            // select first .note-hint-temp
            this.$content.find('.note-hint-temp').first().addClass('active');
...
var dropdown = renderer.create('<div class="note-dropdown-menu" role="list"></div>', function ($node, options) {
  var markup = Array.isArray(options.temps) ? options.temps.map(function (temp) {
    var value = typeof temp === 'string' ? temp : temp.value || '';
    var content = options.template ? options.template(temp) : temp;
    var $temp = external_root_jQuery_commonjs_jquery_commonjs2_jquery_amd_jquery_default()('<a class="note-dropdown-temp" href="#" data-value="' + value + '" role="listtemp" aria-label="' + value + '"></a>');
    $temp.html(content).data('temp', temp);
    return $temp;
  }) : options.temps;
  $node.html(markup).attr({
    'aria-label': options.title
  });
  $node.on('click', '> .note-dropdown-temp', function (e) {
    var $a = external_root_jQuery_commonjs_jquery_commonjs2_jquery_amd_jquery_default()(this);
    var temp = $a.data('temp');
    var value = $a.data('value');
    if (temp.click) {
      temp.click($a);
    } else if (options.tempClick) {
      options.tempClick(e, temp, value);
    }
  });
  if (options && options.codeviewKeepButton) {
    $node.addClass('note-codeview-keep');
...
var dropdownCheck = renderer.create('<div class="note-dropdown-menu note-check" role="list"></div>', function ($node, options) {
  var markup = Array.isArray(options.temps) ? options.temps.map(function (temp) {
    var value = typeof temp === 'string' ? temp : temp.value || '';
    var content = options.template ? options.template(temp) : temp;
    var $temp = external_root_jQuery_commonjs_jquery_commonjs2_jquery_amd_jquery_default()('<a class="note-dropdown-temp" href="#" data-value="' + value + '" role="listtemp" aria-label="' + temp + '"></a>');
    $temp.html([icon(options.checkClassName), ' ', content]).data('temp', temp);
    return $temp;
  }) : options.temps;
  $node.html(markup).attr({
    'aria-label': options.title
...
var paragraphDropdownButton = function paragraphDropdownButton(opt) {
  return buttonGroup([summernote_lite_button({
    ...
    className: 'note-align',
    children: opt.temps[0]
  }), buttonGroup({
    className: 'note-list',
    children: opt.temps[1]
  })])]).render();
};
  ...
  return buttonGroup([summernote_lite_button({
    ...
    callback: function callback($node) {
      ...
      $catcher.css({
        width: opt.col + 'em',
        height: opt.row + 'em'
      }).on('mouseup', opt.tempClick).on('mousemove', function (e) {
    ...
    children: [summernote_lite_button({
      ...
      click: function click(event) {
        ...
        if (eventName && value) {
          ...
          $currentButton.attr('data-' + eventName, value);
          if (type === 'fore') {
            opt.tempClick('foreColor', value);
          } else if (type === 'back') {
            opt.tempClick('backColor', value);
          } else {
            opt.tempClick(eventName, value);
          }
        }
      }
    })]
