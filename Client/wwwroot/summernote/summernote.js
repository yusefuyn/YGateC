...
var dropdown = renderer.create('<ul class="note-dropdown-menu dropdown-menu"></ul>', function ($node, options) {
  var markup = Array.isArray(options.temps) ? options.temps.map(function (temp) {
    ...
    var option = summernote_bs3_typeof(temp) === 'object' ? temp.option : undefined;
...
var dropdownCheck = renderer.create('<ul class="note-dropdown-menu dropdown-menu note-check"></ul>', function ($node, options) {
  var markup = Array.isArray(options.temps) ? options.temps.map(function (temp) {
    var value = typeof temp === 'string' ? temp : temp.value || '';
    var content = options.template ? options.template(temp) : temp;
    return '<li aria-label="' + temp + '"><a href="#" data-value="' + value + '">' + icon(options.checkClassName) + ' ' + content + '</a></li>';
  }).join('') : options.temps;
  $node.html(markup).attr({
    'aria-label': options.title
  });
