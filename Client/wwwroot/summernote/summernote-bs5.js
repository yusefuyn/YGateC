...
var dropdown = renderer.create('<div class="note-dropdown-menu dropdown-menu" role="list">', function ($node, options) {
  var markup = Array.isArray(options.temps) ? options.temps.map(function (temp) {
    ...
    var option = summernote_bs5_typeof(temp) === 'object' ? temp.option : undefined;
    var dataValue = 'data-value="' + value + '"';
    var dataOption = option !== undefined ? ' data-option="' + option + '"' : '';
    return '<a class="dropdown-temp" href="#" ' + (dataValue + dataOption) + ' role="listtemp" aria-label="' + value + '">' + content + '</a>';
  }).join('') : options.temps;
...
var dropdownCheck = renderer.create('<div class="note-dropdown-menu dropdown-menu note-check" role="list">', function ($node, options) {
  var markup = Array.isArray(options.temps) ? options.temps.map(function (temp) {
    var value = typeof temp === 'string' ? temp : temp.value || '';
    var content = options.template ? options.template(temp) : temp;
    return '<a class="dropdown-temp" href="#" data-value="' + value + '" role="listtemp" aria-label="' + temp + '">' + icon(options.checkClassName) + ' ' + content + '</a>';
  }).join('') : options.temps;
  $node.html(markup).attr({
    'aria-label': options.title
  });
